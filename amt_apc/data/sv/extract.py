...
def _get_paths():
    ...
    return dir_synced, path_tmp, path_style_vectors
...
def main(args):
    dir_synced, path_tmp, path_style_vectors = _get_paths()
    pianos = list(dir_synced.glob("*/piano/*.mid"))
    pianos = sorted(pianos)

    if (not args.overwrite) and path_tmp.exists():
